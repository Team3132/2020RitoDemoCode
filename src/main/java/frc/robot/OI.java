...
public class OI {
  ...
  public OI(
		 ...
		 */
    new JoystickButton(driverGamePad, 1)
      .whileHeld(new WidgetMoveUpCommand(widgetSubsystem, 0.2))
      .whenReleased(new InstantCommand(widgetSubsystem::setMoveStop, widgetSubsystem));
		new JoystickButton(driverGamePad, 2).whileHeld(new WidgetMoveDownCommand(widgetSubsystem, 0.2));
		new JoystickButton(driverGamePad, 2).whenReleased(new InstantCommand(widgetSubsystem::setMoveStop, widgetSubsystem));
		
		new JoystickButton(operatorGamePad, GamepadButtons.A_BUTTON).whenPressed(new InstantCommand(widgetSubsystem::setMoveStop, widgetSubsystem));
		
		/*
		 * When the demo trigger conditions are met then we run the WidgetStopCommand()
		 */
		new DemoTrigger(operatorGamePad, 3, widgetSubsystem).whenActive(new InstantCommand(widgetSubsystem::setMoveStop, widgetSubsystem));
	}
}
